...
const MyDash = ({ user, onLogout }: Props) => {
  const [activeTab, setActiveTab] = useState('Home');
  const avatarInitial = user.email ? user.email[0].toUpperCase() : 'U';

  if (activeTab === 'Payments') return <Payments activeTab={activeTab} setActiveTab={setActiveTab} />;
  if (activeTab === 'Children') return <Children activeTab={activeTab} setActiveTab={setActiveTab} />;
  if (activeTab === 'Setting') return <Setting activeTab={activeTab} setActiveTab={setActiveTab} user={user} onLogout={onLogout} />;
      ...
      <View style={shared.header}>
        <View style={shared.headerLeft}>
          <View style={shared.avatar}>
            <Text style={shared.avatarText}>{avatarInitial}</Text>
          </View>
          <Text style={shared.greeting}>Hello, {user.email?.split('@')[0]}!</Text>
        </View>
